...
explainer <- explain_tidymodels(penguin_fit,
                                data = penguin_test,
                                y = penguin_test$sex,
                                label = "tidymodels")

new_penguins <- penguin_test %>% slice(1:3) %>% as.data.frame()
rownames(new_penguins) <- c("penguin_1", "penguin_2", "penguin_3")

modelStudio(explainer, new_penguins,  B = 50)
